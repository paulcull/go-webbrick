...
type Device struct {
	...
	Level			  float		   // What is the level of the device
...
// Don't need to subscribe for webbricks, but if we did, we'd do it here
// func Subscribe() {
...
// Query asks all the devices we know about, for their names. 
// Have a different approach to query - don't need for the webbrick
...
func SetState(devID string, state bool) (bool, error) {
	...
	case: LIGHT 							 	
		...
		if state == true && {
			...
			Devices[devID].Level = "00"
		}
		// create and send the command
		var command = 'http://'+Devices[devID].State+'/hid.spi?AA'+Devices[devID].Channel+'='+Devices[devID].Level
		...
		passMessage("lightset:"+Devices[devID].Level, *Devices[macAdd])
		return success, err

	// Its a fake button to trigger the DI	
	case: BUTTON 
		// create and send the command
		var command = 'http://'+Devices[devID].State+'/hid.spi?DI'+Devices[devID].Channel
		success, err := sendCommand(command, devID)
		//success, err := sendMessage("686400176463"+macAdd+twenties+"00000000"+statebit, Devices[macAdd].IP)
		passMessage("button", *Devices[macAdd])
		return success, err

	// don't know what to do or how to do it
	default:
		return false, errors.New("Can't set state on "+Devices[devID].Type)
	}
...
func handleMessage(message string, addr *net.UDPAddr) (bool, error) {
	...
	Devices[devID].LastMessage = message // Set our LastMessage
	return true, nil
}
...
//		success, err := sendCommand(command, devID)
func sendCommand(command string, devID) (bool, error) {
