...
func main() {
	...
	// sent, err := publishMessage("TEST", "webbrick")
	...
	if ready == true {                              // Yep! Let's do this!
		for { // Loop forever
			...
			case msg := <-webbrick.Events:
				...
				//sent, err := publishMessage("TEST", "webbrick/"+strconv.Itoa(msg.DeviceInfo.ID)+"/"+msg.Name+"/"+msg.DeviceInfo.DevID)
				//sent, err := publishMessage("TEST", "webbrick")
				if err != nil && sent == false {
					fmt.Println(" !!!!!!!!!!!!!!!!! Error in publishMessage")
					panic(err)
	...
	// fmt.Println("Setting up mqtt client...Dis-connecting...done")	// // Disconnect the Network Connection.
	// if err := cli.Disconnect(); err != nil {
	// 	panic(err)
	// }
